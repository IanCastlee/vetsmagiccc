...
const ShopReservationHistory = () => {
    ...
    const fetchReservations = async () => {
      try {
        const res = await axiosIntance.get(
          "admin/shop/get_shop_reservation.php?status=1"
        );
        if (res.data.success) {
  ...
  return (
    <div className="admin-appointment">
      <div className="top">
        <div className="left">
          <h3>SHOP RESERVATION HISTORY</h3>
        </div>
        <div className="right">
      ...
      <div className="table">
        ...
        >
          ...
          <MdOutlinePending
            onClick={() => navigate("/admin/shop-reservation")}
            ...
            {currentData && currentData.length > 0 ? (
              currentData.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{item.qty}</td>
                  <td>{item.note}</td>
                  <td style={{ color: item.status === 0 ? "red" : "green" }}>
                    {item.status === 0 ? (
                      <button
...
export default ShopReservationHistory;
